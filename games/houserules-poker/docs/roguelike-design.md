# Game Design Document: House Rules Roguelike Mode
...
- **Poker, Evolved** – The foundation remains Texas Hold'em. Every decision still matters.
- **Roguelike Runs** – Each session is a standalone run with escalating chaos and asymmetry.
- **Hidden Powers** – Players draft Relics that are face-down until revealed through use.
- **Dynamic Law** – Every table evolves different "house rules" as the run progresses.
- **Short, Complete Loops** – Each run is a full arc: setup → escalation → showdown → reset.
...
- **Activation**: Player clicks → confirm → animation + broadcast text.
  Example: _"Nathan invoked The Dealer! The flop is reshuffled!"_
...
**Status**: Design document only - not yet implemented. Saved for future development phase.
